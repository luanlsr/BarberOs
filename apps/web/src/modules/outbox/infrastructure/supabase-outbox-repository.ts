...
const jobSelect =
  'id, tenant_id, branch_id, type, status, schema_version, source_type, source_id, outbox_event_id, notification_intent_id, payload, idempotency_key, correlation_id, priority, attempt_count, max_attempts, run_at, locked_by, locked_until, last_error, completed_at, created_at, updated_at';
const attemptSelect =
  'id, tenant_id, branch_id, job_id, outbox_event_id, status, attempt_number, worker_id, started_at, finished_at, error';
...
export class SupabaseOutboxRepository implements OutboxRepository, WorkerJobRepository {
  ...
  async claimAvailableJobs(input: WorkerJobClaimInput) {
    const now = input.now ?? new Date();
    const leaseTtlMs = input.leaseTtlMs ?? 60_000;
    const { data, error } = await this.client.rpc('claim_worker_jobs', {
      p_worker_id: input.workerId,
      p_limit: input.limit ?? 10,
      p_lease_until: new Date(now.getTime() + leaseTtlMs).toISOString(),
...
function toEventRow(command: CreateOutboxEventCommand) {
  return {
    ...
    available_at: command.availableAt ?? new Date().toISOString(),
    schema_version: 1,
    status: 'PENDING',
  };
}
...
function toJobRow(command: CreateWorkerJobCommand) {
  return {
    ...
    source_type: command.sourceType,
    source_id: command.sourceId,
    outbox_event_id: command.outboxEventId,
    ...
    run_at: command.runAt ?? new Date().toISOString(),
    status: 'PENDING',
  };
}
...
export function toWorkerJob(row: Record<string, unknown>): WorkerJob {
  return workerJobSchema.parse({
    ...
    outboxEventId: row.outbox_event_id ?? undefined,
...
export function toWorkerJobAttempt(row: Record<string, unknown>): WorkerJobAttempt {
  return workerJobAttemptSchema.parse({
    id: row.id,
    tenantId: row.tenant_id,
    branchId: row.branch_id ?? undefined,
    jobId: row.job_id,
